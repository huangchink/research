...
from model.twoeyenet_transformer_1054_gray import DGMnet  # 使用 DGMnet
...
def load_pretrained_backbone(model, pretrained_path):
    ...
    valid_pretrained_dict = {}
    # 遍历所有预训练权重，检查匹配情况
    for k, v in pretrained_dict.items():
        if k not in model_dict:
            print(f"Skipping layer '{k}': not present in the model.")
        elif v.size() != model_dict[k].size():
            print(f"Skipping layer '{k}': size mismatch (pretrained: {v.size()}, model: {model_dict[k].size()}).")
        else:
            valid_pretrained_dict[k] = v

    # 更新模型权重
    model_dict.update(valid_pretrained_dict)
    model.load_state_dict(model_dict)
    print(f"Loaded {len(valid_pretrained_dict)} layers from pretrained model.")
...
if __name__ == "__main__":
    ...
    for fold in range(3):
        ...
        val_dataset = dataloader.txtload(val_file, imagepath, 56, num_workers=4, header=True)
        ...
        pretrained_path = "/home/tchuang/research/RTGENE/loadmodel/gray_1_37.pt"
