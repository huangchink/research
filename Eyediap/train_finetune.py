...
if __name__ == "__main__":
    ...
    # 用來存放每個 epoch (從 5 到最終 epoch) 各 fold 的驗證誤差
    ...
    for fold in range(len(all_labels)):  # 總共 4 folds
        ...
        train_dataset = dataloader.txtload(train_labels, imagepath, config["params"]["batch_size"], shuffle=True, header=True)
        val_dataset = dataloader.txtload(val_label, imagepath, 16, header=True)
    ...
    for epoch in sorted(epoch_validation_errors.keys()):
        ...
        last_avg_error=avg_val_error

    print(f"\n=== 最終 3-Fold Cross Validation:  average error: {last_avg_error:.2f} degrees ===")