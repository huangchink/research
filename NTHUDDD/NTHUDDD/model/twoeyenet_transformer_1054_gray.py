...
import numpy as np
from Harnet_gray import HarDNetFeatureExtractor
...
class EyeImageModel(nn.Module):
    def __init__(self):
        ...
        self.features1_1 = nn.Sequential(
            nn.Conv2d(1, 64, 3, 1, 1, dilation=(2, 2)),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            CBAM(64),
...
class Transformer(nn.Module):
    ...
    def forward(self, feature, num = 1):
        ...
        for i in range(num):
            ...
            # position
            position = torch.arange(self.length, device=feature.device)
            pos_feature = self.pos_embedding(position)

            token_position=torch.tensor([i], device=feature.device, dtype=torch.long)
...
class DGMnet(nn.Module):
    def __init__(self):
        ...
        # 加載預訓練的 HarDNet
        self.base_model = HarDNetFeatureExtractor(arch=68, pretrained=False)
        # 根據 HarDNet 的輸出設置特徵金字塔
        self.MFSEA = MFSEA(input_nums=[124,654,128,128], hidden_num=128)
        # 微調輸出金字塔特徵
        self.transformer = Transformer(input_dim=128, nhead=16, hidden_dim=256,
                                       layer_num=6, pred_num=1, length=len([124, 262,128,128]))
    ...
    def forward(self, x_in):
        ...
        multilevelfeature, feature_list = self.MFSEA([features[0],features[3],xEyeL,xEyeR])
        ...
        return clstoken_fusion_output,gaze,gaze_list[0],gaze_list[1],gaze_list[2],gaze_list[3]
