...
# -----------------------------------------------------------------------------
# Positional Encoder (非 nn.Module, 直接透過 encode 方法)
# -----------------------------------------------------------------------------
...
class DMSnet(nn.Module):
    def __init__(self, nhead=5, num_layers=6, channel_num_layers=6, seq_length=30, out_dim=2):
        """
        Args:
        ...
        # 每個 frame token 由 gaze (2) + head pose (3) + EAR (2) + mar 組成，共8維
        self.token_dim = 5
        self.seq_length = seq_length

        # # 時間流 Transformer
        # self.pos_encoder = PositionalEncoder(number_freqs=2, include_identity=True)
        # # 由於 include_identity=True 與 number_freqs=3，輸出維度 = 1 + 2*3 = 7
        # print(self.pos_encoder.getDims() )
        # assert self.pos_encoder.getDims() == self.token_dim, "PositionalEncoder output dim must equal token_dim (7)"

        self.pos_embedding = nn.Embedding(seq_length, self.token_dim)


        temporal_layer = nn.TransformerEncoderLayer(d_model=self.token_dim, nhead=nhead, dropout=0.4, dim_feedforward=10)
        self.temporal_encoder = nn.TransformerEncoder(temporal_layer, num_layers=num_layers)

        # 通道流 Transformer
        # 將原始 token (B, seq, 8) 轉換成 (B, 8, seq)
        # 這裡我們設定 d_model = seq_length (例如 30)
        channel_layer = nn.TransformerEncoderLayer(d_model=self.seq_length, nhead=nhead, dropout=0.4, dim_feedforward=10)
        ...
        # Fusion transformer encoder：融合整個 token 序列（包含時間流和通道流 tokens)
        # 分類頭：利用 class token 輸出結果
    ...
    def forward(self, x):
        ...
        ear_dyn   = ear_avg - ear_shift         
        token = torch.cat((gaze,ear_avg,ear_dyn, mar), dim=-1)
        
        # --------------------------
        # 時間流處理
        # --------------------------
        token_t = token.transpose(0, 1)  # (seq, B, 8)
        ...
        # pos_embedding = self.pos_encoder.encode(pos_input).unsqueeze(1)  # (seq, 1, 8)
        token_t = token_t + pos_emb
        T = self.temporal_encoder(token_t)  # (seq, B, 8)
        T = T.transpose(0, 1)  # (B, seq, 8)
        
        # --------------------------
        # 通道流處理
        # --------------------------
        channel_tokens = token.transpose(1, 2)  # (B, 8, seq)
        channel_tokens = channel_tokens.transpose(0, 1)  # (8, B, seq)
        C_out = self.channel_transformer(channel_tokens)  # (8, B, seq)
        #print(C_out.shape)
        C_out = C_out.transpose(0, 1)  # (B, 8, seq)
        

        T_proj = T.reshape(B,1, seq*self.token_dim)
        #print(T_proj.shape)

        C_proj = C_out.reshape(B,1, seq*self.token_dim)
        #print(C_proj.shape) 
        ...
        cls        = self.cls_token.expand(B,1,D).transpose(0,1)       # (1,B,D)

        # Cross-Attention
        attn_out, _ = self.cross_attn(cls, seq_tokens, seq_tokens)   # (1,B,D)
        attn_out = attn_out.squeeze(0)                                # (B,D)

         # 殘差＋FFN（Pre-LN）
        y  = self.norm1(attn_out + cls.squeeze(0))  # (B,D)
...
if __name__ == '__main__':
    ...
    B = 1
    seq = 30  # 必須與模型的 seq_length 參數一致
    dummy_input = {
        "origin_face": torch.zeros(B, seq, 1, 224, 224).to(device),
        "left_eye": torch.zeros(B, seq, 1, 112, 112).to(device),
        "right_eye": torch.zeros(B, seq, 1, 112, 112).to(device),
        "head_pose": torch.zeros(B, seq, 1).to(device),
