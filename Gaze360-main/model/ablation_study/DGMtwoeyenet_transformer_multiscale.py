...
class DGMnet(nn.Module):
    def __init__(self):
        ...
        self.downsample=conv1x1(124,128)
        self.downsample2=conv1x1(654,128)

        self.transformer = Transformer(input_dim=128, nhead=16, hidden_dim=256,
                                       layer_num=6, pred_num=1, length=len([124,128, 262,128]))
    ...
    def forward(self, x_in):
        ...
        face_feature_low=self.downsample(face_features[0])
        face_feature_high=self.downsample2(face_features[3])

        xEyeL=self.avgpooling(xEyeL)
        xEyeR=self.avgpooling(xEyeR)
        xEyeL=xEyeL.flatten(1)
        xEyeR=xEyeR.flatten(1)
        face_feature_low=face_feature_low.unsqueeze(0)
        face_feature_high=face_feature_high.unsqueeze(0)
        ...
        # print(xEyeL.shape)
        # print(xEyeR.shape)
        fusion_input = torch.cat([face_feature_low,face_feature_high, xEyeL,xEyeR], dim=0)
