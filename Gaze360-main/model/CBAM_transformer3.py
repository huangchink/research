...
#     def forward(self, x):
#         x1 = self.features1_3(self.features1_2(self.features1_1(x)))# 預期輸出尺寸[B, 16, 7, 7] 
#         return x1
...
#         self.eyeModel   = EyeImageModel()
#         self.base_model = HarDNetFeatureExtractor(arch=68, depth_wise=True, pretrained=True)
...
#         self.pos_embedding = nn.Embedding(4, self.d_model)
#         self.fc = nn.Sequential(
#             nn.Linear(self.d_model, 2)
#         )
#         # 結合臉部特徵後進行簡單卷積處理
#         self.linearPJ_global = ProjectionModule(global_channel,self.d_model)
#         # self.linearPJ_local  = ProjectionModule( local_channel,self.d_model)
...
#         # 1. Face 網路
#         face_feat = self.base_model(x_in['origin_face'])# => [B, 1024, 7,7]
#         # 2. Eye 網路
#         eyeL = self.eyeModel(x_in['left_eye'])  # e.g. [B, 64, 7,7]
#         eyeR = self.eyeModel(x_in['right_eye']) # e.g. [B, 64, 7,7]

#         #align global and local features after Projection
#         face_feat = self.linearPJ_global(face_feat)     #      [B, 64, 7,7]
#         # eyeL      = self.linearPJ_local(eyeL)# e.g. [B, 64, 7,7]
#         # eyeR      = self.linearPJ_local(eyeR)# e.g. [B, 64, 7,7]
        

#         face_feat=self.global_pool(face_feat)    
#         eyeL=self.global_pool(eyeL)# e.g. [B, 64, 1,1]
#         eyeR=self.global_pool(eyeR)# e.g. [B, 64, 1,1]

#         face_feat = face_feat.flatten(2)                  # => [B, 64, 1]
#         eyeL = eyeL.flatten(2)                  # => [B, 64, 1]
#         eyeR = eyeR.flatten(2)                  # => [B, 64, 1]

#         #using transformer to fuse global and local features
#         # 加 CLS token：形狀 [B, 1, 49]
#         cls = self.cls_token.repeat(B, 1, 1)                # => [B, 64, 1]
#         fusion_input = torch.cat([cls, face_feat,eyeL,eyeR], dim=2)# => [B, 64, 4]
#         # Transformer 需 [seq_len, batch_size, d_model] => permute
#         fusion_input = fusion_input.permute(2, 0, 1)        # => [4, B, 64]
#         pos_idx     = torch.arange(0, 4, device=fusion_input.device)  # [0..3]
#         pos_feature = self.pos_embedding(pos_idx)           # => [4, 64]

#         # 送入 Transformer
#         output = self.encoder(fusion_input, pos_feature)    # => [4, B, 64]

#         # 取 CLS token (sequence 維度 = 0) => [B, 49]
#         cls_feature = output[0]                             # => [B, 64]
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=512, dropout=0.1):
        ...
        self.norm2 = nn.LayerNorm(d_model)
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.activation = nn.GELU()  # 可嘗試使用 GELU 替代 ReLU
        self.dropout2 = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
        self.dropout3 = nn.Dropout(dropout)
    
    def forward(self, src, pos):
        # 先加上位置編碼，再做 Layer Norm
        src2 = self.norm1(src + pos.unsqueeze(1).repeat(1, src.size(1), 1))
        # 注意：需確保位置編碼處理方式與輸入 shape 匹配
...
class DGMnet(nn.Module):
    ...
    def forward(self, x_in):
        ...
        eyeL = self.eyeModel(x_in['left_eye'])  # e.g. [B, 64, 7,7]
        eyeR = self.eyeModel(x_in['right_eye']) # e.g. [B, 64, 7,7]
        fusion_input = torch.cat([features, eyeL, eyeR], dim=1)  # => [B, 1056, 7,7]
        face_feat = self.linearPJ(fusion_input)                    # [B, 32,   7, 7]
        face_feat = face_feat.flatten(2)                  # => [B, 32, 49]

        # 加 CLS token：形狀 [B, 1, 49]
        cls = self.cls_token.repeat(B, 1, 1)                # => [B, 32, 1]
        fusion_input = torch.cat([cls, face_feat], dim=2)# => [B, 32, 50]
        #print(fusion_input.shape)

        # Transformer 需 [seq_len, batch_size, d_model] => permute
        fusion_input = fusion_input.permute(2, 0, 1)        # => [50, B, 128]

        # Positional embedding: arange(129)
        pos_idx     = torch.arange(0, 50, device=fusion_input.device)  # [0..49]
        pos_feature = self.pos_embedding(pos_idx)           # => [129, 49]

        # 送入 Transformer
        output = self.encoder(fusion_input, pos_feature)    # => [129, B, 49]
