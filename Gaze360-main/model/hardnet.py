...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path=''):
        ...
        if pretrained:
            ...
            if arch == 68 and depth_wise:
                checkpoint = '/home/tchuang/research/Gaze360-main/model/hardnet68ds.pth'
            else:
                checkpoint = '/home/tchuang/research/Gaze360-main/model/hardnet68.pth'
            self.load_state_dict(torch.load(checkpoint), strict=False)
            print("Loaded pretrained HarDNet weights (feature extraction only).")
    ...
    def forward(self, x):
        ...
        for layer in self.base:
            ...
            # if isinstance(layer, HarDBlock):
            #     features.append(x)  # 儲存每個 HarDBlock 的輸出
            #     print(f'x shape:',x.shape)
...
class HarDNetFeatureExtractor(nn.Module):
    def __init__(self, depth_wise=False, arch=85, pretrained=True, weight_path=''):
        super().__init__()
        first_ch  = [32, 64]
        second_kernel = 3
        ...
        # 只保留特徵提取部分，不加入 MLP 預測層

        if pretrained:
            checkpoint = './model/hardnet68.pth'
            self.load_state_dict(torch.load(checkpoint, map_location=torch.device('cpu')), strict=False)
            #print("Loaded pretrained HarDNet weights (feature extraction only).")
...
if __name__ == '__main__':
    base_model = HarDNetFeatureExtractor(arch=68, pretrained=True)
    feature = {
    ...
    base_model.cuda()
    features = base_model(feature['origin_face'])
